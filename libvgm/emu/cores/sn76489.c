...
/* Jupiter SDK wrappers */
void *sn76489_jupiter_init(UINT32 clock, UINT32 rate)
{
    SN76489_Context *chip = SN76489_Init(clock, rate);
    if (chip) chip->Mute = 0x0F; /* all 4 channels ON (1=active) */
    return chip;
}
void sn76489_jupiter_shutdown(void *chip) { SN76489_Shutdown((SN76489_Context*)chip); }
void sn76489_jupiter_reset(void *chip) { SN76489_Reset((SN76489_Context*)chip); }
void sn76489_jupiter_write(void *chip, UINT8 data) { SN76489_Write((SN76489_Context*)chip, data); }
void sn76489_jupiter_update(void *chip, UINT32 len, INT32 *left, INT32 *right)
{
    DEV_SMPL *bufs[2] = { left, right };
    SN76489_Update((SN76489_Context*)chip, len, bufs);
}


/* Jupiter debug accessor */
void sn76489_jupiter_debug(void *chip)
{
    extern void uart_puts(const char *);
    extern void uart_putdec(unsigned int);
    extern void uart_puthex(unsigned int);
    SN76489_Context *ctx = (SN76489_Context *)chip;
    if (!ctx) return;
    uart_puts("  ---- SN76489 PSG STATE ----\n");
    int i;
    for (i = 0; i < 4; i++) {
        uart_puts("  CH"); uart_putdec(i);
        uart_puts(": tone_reg="); uart_putdec(ctx->Registers[i*2] & 0x3FF);
        uart_puts(" vol_reg="); uart_putdec(ctx->Registers[i*2+1] & 0x0F);
        uart_puts(" freq_cnt="); uart_putdec(ctx->ToneFreqVals[i] & 0xFFFF);
        uart_puts(" pos="); uart_putdec(ctx->ToneFreqPos[i] & 0xFFFF);
        uart_puts(" mute="); uart_putdec((ctx->Mute >> i) & 1);
        uart_puts("\n");
    }
    uart_puts("  NoiseReg="); uart_puthex(ctx->NoiseFreq);
    uart_puts(" LFSR="); uart_puthex(ctx->NoiseShiftRegister);
    uart_puts("\n");
}
void sn76489_jupiter_unmute(void *chip)
{
    SN76489_Context *c = (SN76489_Context *)chip;
    if (c) c->Mute = 0x0F;  /* all 4 channels ON (1=active, not 0!) */
}
