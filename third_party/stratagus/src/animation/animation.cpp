...
/* Jupiter bare-metal port: .init_array isn't run, so TU-global std::maps
 * live in zero-initialized .bss and blow up on first mutation. Wrapping in
 * a function-local static defers construction to first use. Same trick we
 * use for MissileClassNames in script_missile.cpp. */
static auto& AnimationMap() {
	static std::map<std::string, std::unique_ptr<CAnimations>, std::less<>> m;
	return m;
}
...
{
	uart_puts("[LL] push name='"); uart_puts(name.c_str()); uart_puts("' len=");
	{ char b[12]; int n=(int)name.size(), j=10; b[11]=0; if(!n)b[j--]='0'; while(n){b[j--]='0'+n%10;n/=10;} uart_puts(&b[j+1]); }
	uart_puts("\n");
...
{
	uart_puts("[FX] LabelsLater.size="); { char b[12]; int n=(int)LabelsLater.size(), j=10; b[11]=0; if(!n)b[j--]='0'; while(n){b[j--]='0'+n%10;n/=10;} uart_puts(&b[j+1]); } uart_puts("\n");
	int idx = 0;
	for (auto &labelLater : LabelsLater) {
		uart_puts("[FX] "); { char b[12]; int n=idx++, j=10; b[11]=0; if(!n)b[j--]='0'; while(n){b[j--]='0'+n%10;n/=10;} uart_puts(&b[j+1]); } uart_puts(" name="); uart_puts(labelLater.Name.c_str()); uart_puts("\n");
...
**  Parse an animation
*/
/* Thin progress indicator — brigand's Move animation has ~2000 entries and
 * QEMU under icount takes multi-second wall-clock to parse. Print a dot per
 * 200 entries so we can see progress vs a real hang. */
...
static std::vector<std::unique_ptr<CAnimation>> ParseAnimation(lua_State *l, int idx)
{
	...
	auto utoa = [](uint32_t n, char *buf) {
		char tmp[12]; int j = 10; tmp[11] = 0;
		if (n == 0) { tmp[j--] = '0'; }
		while (n) { tmp[j--] = '0' + (n % 10); n /= 10; }
		char *s = &tmp[j + 1]; while (*s) *buf++ = *s++;
		*buf = 0;
	};
	char nbuf[12], hbuf[12];
	utoa((uint32_t)args, nbuf);
	utoa(libc_shim_heap_used(), hbuf);
	uart_puts("[PA] n="); uart_puts(nbuf); uart_puts(" heap="); uart_puts(hbuf); uart_puts("\n");
	...
	utoa(libc_shim_heap_used(), hbuf);
	uart_puts("[PA] parsed heap="); uart_puts(hbuf); uart_puts("\n");

	FixLabels(l, animations);
	uart_puts("[PA] post-FixLabels\n");
	auto r = std::move(animations);
	uart_puts("[PA] post-move\n");
	return r;
}
...
static int CclDefineAnimations(lua_State *l)
{
	...
	while (lua_next(l, 2)) {
		...
		uart_puts("[DA2] field="); uart_puts(std::string(value).c_str()); uart_puts("\n");
		...
		lua_pop(l, 1);
		uart_puts("[DA2] loop iter done\n");
	}
	uart_puts("[DA2] loop exited\n");
