...
{
	/* Jupiter bare-metal: istringstream >> is broken without static-init
	 * locale. Split manually on whitespace. */
	auto next_token = [](std::string_view &sv) -> std::string {
		const size_t b = sv.find_first_not_of(' ');
		if (b == std::string_view::npos) { sv = {}; return {}; }
		const size_t e = sv.find(' ', b);
		std::string tok = (e == std::string_view::npos)
			? std::string(sv.substr(b))
			: std::string(sv.substr(b, e - b));
		sv = (e == std::string_view::npos) ? std::string_view{} : sv.substr(e);
		return tok;
	};
	std::string_view rem = s;
	this->leftVar = next_token(rem);
	std::string op = next_token(rem);
	this->rightVar = next_token(rem);
	std::string label = next_token(rem);
