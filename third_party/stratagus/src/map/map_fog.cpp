...
/* Bare-metal diag helpers for the fog probes below. */
extern "C" {
...
{
	static int fog_probe = 0;
	if (!fog_probe) {
		fog_probe = 1;
		uart_puts("[FOG] DrawMapFogOfWar entered ReplayRevealMap=");
		uart_putdec((unsigned)ReplayRevealMap);
		uart_puts(" FogOfWar=");
		uart_putdec(FogOfWar != nullptr);
		uart_puts(" FogSurface=");
		uart_putdec(this->FogSurface != nullptr);
		uart_puts(" TL=");
		uart_putdec((unsigned)this->TopLeftPos.x); uart_puts(",");
		uart_putdec((unsigned)this->TopLeftPos.y);
		uart_puts(" BR=");
		uart_putdec((unsigned)this->BottomRightPos.x); uart_puts(",");
		uart_putdec((unsigned)this->BottomRightPos.y);
		uart_puts("\n");
	}
	...
	if (isSoftwareRender && FogOfWar->GetType() != FogOfWarTypes::cTiledLegacy) {
		...
		static int fog_blit_probe = 0;
		if (fog_blit_probe < 3) {
			fog_blit_probe++;
			uart_puts("[FOG] composite srcRect=");
			uart_putdec((unsigned)fogRect.x); uart_puts(",");
			uart_putdec((unsigned)fogRect.y); uart_puts(",");
			uart_putdec((unsigned)fogRect.w); uart_puts(",");
			uart_putdec((unsigned)fogRect.h);
			uart_puts(" fogSurfaceWxH=");
			uart_putdec((unsigned)this->FogSurface->w); uart_puts("x");
			uart_putdec((unsigned)this->FogSurface->h);
			/* Sample 5 pixels along the diagonal to see what's actually
			 * in FogSurface. If all 0x00000000, the upscale path didn't
			 * write anything. If 0xFF303030, fog data is fine and the
			 * blit must be the broken link. */
			const uint32_t *fp = (const uint32_t *)this->FogSurface->pixels;
			int stride = this->FogSurface->pitch / 4;
			for (int i = 0; i < 5; i++) {
				int sx = (this->FogSurface->w * i) / 5;
				int sy = (this->FogSurface->h * i) / 5;
				uint32_t pix = fp[sy * stride + sx];
				uart_puts(" px[");
				uart_putdec((unsigned)sx); uart_puts(",");
				uart_putdec((unsigned)sy); uart_puts("]=");
				uart_puthex(pix);
			}
			uart_puts("\n");
		}
