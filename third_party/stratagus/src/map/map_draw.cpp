...
/* Bare-metal diag helpers for the viewport-draw probe below. */
#include <cstdint>
extern "C" {
...
void CViewport::Draw(const fieldHighlightChecker highlightChecker /* = nullptr */)
{
	...
	CurrentViewport = this;
	{
		...
		{
			static int draw_probe = 0;
			if (draw_probe < 5) {
				draw_probe++;
				uart_puts("[VP] Draw nunits=");
				uart_putdec((unsigned)nunits);
				uart_puts(" nmissiles=");
				uart_putdec((unsigned)nmissiles);
				uart_puts(" MapPos=");
				uart_putdec((unsigned)this->MapPos.x); uart_puts(",");
				uart_putdec((unsigned)this->MapPos.y);
				uart_puts(" MapWxH=");
				uart_putdec((unsigned)this->MapWidth); uart_puts("x");
				uart_putdec((unsigned)this->MapHeight);
				uart_puts("\n");
			}
		}
