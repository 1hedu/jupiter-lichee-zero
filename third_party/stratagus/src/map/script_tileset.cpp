...
{
	uart_puts("[TS] parse enter\n");
	clear();
	uart_puts("[TS] clear done\n");

	this->pixelTileSize.x = PixelTileSize.x;
	this->pixelTileSize.y = PixelTileSize.y;
	uart_puts("[TS] pixelTileSize="); uart_putdec((unsigned)this->pixelTileSize.x); uart_puts("x"); uart_putdec((unsigned)this->pixelTileSize.y); uart_puts("\n");

	const int args = lua_gettop(l);
	uart_puts("[TS] args="); uart_putdec((unsigned)args); uart_puts("\n");
	...
	for (int j = 1; j < args; ++j) {
		...
		if (value == "name") {
			...
			uart_puts("[TS] parseSlots start\n");
			if (!lua_istable(l, j)) {
				LuaError(l, "incorrect argument");
			}
			parseSlots(l, j);
			uart_puts("[TS] parseSlots done\n");
		} else {
			LuaError(l, "Unsupported tag: %s", value.data());
		}
	}
	uart_puts("[TS] parse exit\n");
}
