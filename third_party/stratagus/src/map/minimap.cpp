...
void CMinimap::Create()
{
	...
	/* Jupiter port: the tileset's SDL surface is a zero-init dummy (we render
	 * tiles via objcopy'd .pc8 blobs). All three minimap surfaces are
	 * unconditionally 32-bit ARGB so UpdateTerrain / UpdateXY (which we
	 * rewrote below to sample from the pc8 sheet) can write Uint32s
	 * directly without a tileset format dependency. */
	MinimapTerrainSurface = SDL_CreateRGBSurface(SDL_SWSURFACE, W, H, 32, RMASK, GMASK, BMASK, AMASK);
...
/* Jupiter port: sample one ARGB pixel from the tileset's pc8 sheet at
 * (gx, gy). Used by UpdateTerrain / UpdateXY to build the minimap mini-
 * bitmap without depending on a real SDL_Surface. Returns 0 (transparent
 * black) if the tileset isn't registered or coords are out of range. */
...
void CMinimap::UpdateTerrain()
{
	...
	const int sheet_w = war1_pc8_image_w_for(Map.TileGraphic.get());
	if (sheet_w <= 0) return;
	const int tilepitch = sheet_w / PixelTileSize.x;
	if (tilepitch <= 0) return;

	const int dst_stride = MinimapTerrainSurface->pitch / 4;
	uint32_t *dst = (uint32_t *)MinimapTerrainSurface->pixels;
	const CGraphic *tg = Map.TileGraphic.get();

	//  Pixel offset 7,6 within each 16x16 tile is the canonical "minimap
	//  representative" sample point upstream uses.
	for (int my = YOffset; my < H - YOffset; ++my) {
		for (int mx = XOffset; mx < W - XOffset; ++mx) {
			...
			const int sx = xofs + 7 + ((mx * SCALE_PRECISION) % scalex) / SCALE_PRECISION * 8;
			const int sy = yofs + 6 + ((my * SCALE_PRECISION) % scaley) / SCALE_PRECISION * 8;
			dst[mx + my * dst_stride] = war1_pc8_sample_argb(tg, sx, sy);
		}
	}
}
