...
/* Bare-metal diag helpers for the HP/burn/poison probes below. */
extern "C" {
...
{
	if (index == HP_INDEX && unit.Variable[index].Increase < 0) {
		static int decay_probe = 0;
		if (decay_probe < 10) {
			decay_probe++;
			uart_puts("[HP-] ");
			uart_puts(unit.Type ? unit.Type->Ident.c_str() : "?");
			uart_puts(" inc=");
			/* Increase is signed; cast to int then unsigned for printing. */
			uart_putdec((unsigned)(int)unit.Variable[index].Increase);
			uart_puts(" hp=");
			uart_putdec((unsigned)unit.Variable[index].Value);
			uart_puts("\n");
		}
	}
...
static bool HandleBurnAndPoison(CUnit &unit)
{
	...
	/* WC1/War1gus port: helpers.lua's `Poison = { Value = 0, Max = 1000,
	 * Enable = true }` is being misparsed via the lua_isnumber dispatch
	 * branch (script_unittype.cpp:1145), leaving every unit spawned with
	 * Value == Max == 1000 (the "spell already cast at full strength"
	 * signature). Real spell-poison via spell_adjustvariable also sets
	 * Value == Max but only briefly; HandleBuffsEachCycle's Increase=-1
	 * decays it. The spawn case never reaches Value < Max. So Value==Max
	 * with non-zero Max is a reliable spawn-bug signature and zeroing it
	 * here doesn't kill spell-poison: the spell will set Value=500
	 * Max=500 again and decay normally. */
	if (unit.Variable[POISON_INDEX].Value > 0
	    && unit.Variable[POISON_INDEX].Value == unit.Variable[POISON_INDEX].Max) {
		unit.Variable[POISON_INDEX].Value = 0;
	}
	// Burn & poison
	...
	if (hpPercent <= unit.Type->BurnPercent && unit.Type->BurnDamageRate) {
		static int burn_probe = 0;
		if (burn_probe < 10) {
			burn_probe++;
			uart_puts("[BURN] ");
			uart_puts(unit.Type ? unit.Type->Ident.c_str() : "?");
			uart_puts(" hp%=");
			uart_putdec((unsigned)hpPercent);
			uart_puts(" BurnPercent=");
			uart_putdec((unsigned)unit.Type->BurnPercent);
			uart_puts(" rate=");
			uart_putdec((unsigned)unit.Type->BurnDamageRate);
			uart_puts("\n");
		}
	...
	if (unit.Variable[POISON_INDEX].Value && unit.Type->PoisonDrain) {
		static int poison_probe = 0;
		if (poison_probe < 10) {
			poison_probe++;
			uart_puts("[POI] ");
			uart_puts(unit.Type ? unit.Type->Ident.c_str() : "?");
			uart_puts(" drain=");
			uart_putdec((unsigned)unit.Type->PoisonDrain);
			uart_puts(" poisonValue=");
			uart_putdec((unsigned)unit.Variable[POISON_INDEX].Value);
			uart_puts(" poisonMax=");
			uart_putdec((unsigned)unit.Variable[POISON_INDEX].Max);
			uart_puts(" poisonEnable=");
			uart_putdec((unsigned)unit.Variable[POISON_INDEX].Enable);
