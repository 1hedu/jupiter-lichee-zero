...
#include "script.h"
/* Forward-decl execvp locally (Jupiter port) — can't #include <unistd.h>
 * because newlib's getopt prototype conflicts with util.h's. */
...
static int report(int status, bool exitOnError)
{
	if (status) {
		...
		ErrorPrint("%s\n", msg);
		/* Jupiter port: never exit on script error. Our bare-metal _exit
		 * just spins, which hangs the boot before we can see anything.
		 * Report + continue like the non-exit path. */
...
/* Jupiter port: implemented in war1_link_stubs.cpp. Walks 0:/save/
 * via FatFs and copies each filename (with any ".gz" suffix stripped)
 * into out[i]. Returns count. */
extern "C" int war1_list_save_dir(char (*out)[64], int max);
...
static int CclFilteredListDirectory(lua_State *l, int type, int mask)
{
	...
	/* Jupiter port: "~save" (the Load Game browser listing) maps to the
	 * FAT partition's save dir. Walk it via FatFs's f_opendir/readdir.
	 * Anything ending in ".gz" is shown without the suffix so the menu
	 * matches the names users actually saved as. */
	if (userdir == "~save" || userdir == "~save/") {
		/* Type 1 = file, 0 = dir. mask filters which kind of entry the
		 * caller wants. Only emit save names when the caller is asking
		 * for files; otherwise return an empty table — the save dir has
		 * no subdirectories. Without this gate, ListDirsInDirectory
		 * also gets the file names back, the Lua browser appends "/"
		 * to each, and the load list ends up with bogus "game.sav/"
		 * pseudo-folder rows. */
		lua_newtable(l);
		if ((type & mask) == (1 & mask) && (mask & 1)) {
			char names[32][64];
			int n = war1_list_save_dir(names, 32);
			for (int j = 0; j < n; j++) {
				lua_pushnumber(l, j + 1);
				lua_pushstring(l, names[j]);
				lua_settable(l, -3);
			}
		}
		return 1;
