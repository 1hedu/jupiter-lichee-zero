...
bool CanAccessFile(const char *filename)
{
	if (filename && filename[0] != '\0') {
		/* On bare-metal there is no writable filesystem — fs::exists
		 * always returns false. Short-circuit through the WC1 asset
		 * catalog so AddFiller / campaign-bg checks in the Lua scripts
		 * see the HUD panels, cursor sheets, etc. that actually got
		 * embedded via the pc8 pipeline. */
		if (war1_asset_exists(filename)) {
			return true;
		}
		static int probe = 0;
		if (probe < 20) {
			probe++;
			uart_puts("[IO] CanAccess MISS file='");
			uart_puts(filename);
			uart_puts("'\n");
		}
