...
{
	/* Jupiter port: spiritless_po's catalog uses static-init machinery
	 * that may not be sound on bare metal — Translate calls during
	 * CContentTypeFormattedText2::Draw (hover info-panel) hung the game.
	 * No translations are loaded in this build anyway, so identity. */
	return str;
}
...
{
	/* Jupiter port: same catalog-bypass as Translate. */
	return str;
}
