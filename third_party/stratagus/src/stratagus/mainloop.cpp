...
/* Bare-metal diag helpers for the heap/cycle probes below. */
extern "C" {
...
void UpdateDisplay()
{
	/* Clear the back fb every frame, not just when a game is running.
	 * The menu branch (no GameRunning) doesn't paint over uncovered
	 * regions, so any widget that exposes screen area outside its own
	 * extent (transparent margin, no-bg menu, etc.) leaves the previous
	 * frame's pixels visible — which presents as cursor trails after
	 * the sprite moves. Hoisting the clear out of the GameRunning
	 * branch costs ~0.5 ms per frame (one 480×272×4 NEON memset) and
	 * removes the "every widget must remember to be opaque" footgun. */
	...
	if (GameRunning || Editor.Running == EditorEditing) {
		...
		static int hud_probe = 0;
		if (!hud_probe) {
			hud_probe = 1;
			uart_puts("[HUD] UpdateDisplay BigMapMode=");
			uart_putdec(BigMapMode ? 1 : 0);
			uart_puts(" Fillers=");
			uart_putdec((unsigned)UI.Fillers.size());
			uart_puts(" InfoPanel.G=");
			uart_putdec(UI.InfoPanel.G != nullptr);
			uart_puts(" ButtonPanel.G=");
			uart_putdec(UI.ButtonPanel.G != nullptr);
			uart_puts(" Minimap.W=");
			uart_putdec((unsigned)UI.Minimap.W);
			uart_puts("\n");
		}
		if (!BigMapMode) {
...
void GameMainLoop()
{
	...
	uart_puts("[GML] entering SingleGameLoop heap=");
	uart_putdec(libc_shim_heap_used());
	uart_puts("\n");
	SingleGameLoop();
