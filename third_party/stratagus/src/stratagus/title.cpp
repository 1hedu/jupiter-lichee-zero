...
void TitleScreen::ShowTitleImage()
{
	...
	/* Jupiter port: scrub the global mouse-button state machine on splash
	 * exit. If the user skipped via button press, our input layer ran
	 * InputMouseButtonPress (because Gui->getTop() was null during splash
	 * → menu_active=false → cb-gated branch fired). The matching release
	 * then takes the menu_active=true branch (because the title menu
	 * shows up before the user releases) which only fires war1_menu_click,
	 * never InputMouseButtonRelease — leaving MouseButtons permanently
	 * holding the LeftButton bit. Downstream selection / drag / marquee /
	 * unit-under-cursor logic then sees "left button perpetually held"
	 * and produces phantom clicks (manifests as scattered unit portraits
	 * after mission load + hangs after start-menu toggles).
	 *
	 * Reset the relevant globals here regardless of skip vs timeout — on
	 * timeout these are already 0 so the writes are harmless. */
	extern unsigned long MouseButtons;
	MouseButtons = 0;
	/* LastMouseButton and MouseState are file-static in interface.cpp;
	 * they self-recover on the next normal Press/Release sequence. The
	 * crucial leak fix is MouseButtons — its LeftButton bit being stuck
	 * is what produces phantom-held-click symptoms downstream. */
}
