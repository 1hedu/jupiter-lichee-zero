...
void CContentTypeLifeBar::Draw(const CUnit &unit, CFont *) const /* override */
{
	...
	// get to right color — guard against malformed values[] (e.g. last entry !=0)
	// to avoid running off the end of the vector.
	const int values_n = (int)this->values.size();
	while (i < values_n && static_cast<unsigned int>(f) < this->values[i]) {
		i++;
	}
	if (i >= values_n) {
		return;
