...
{
	uart_puts("[RMB] DoRightButton mapPx=");
	uart_putdec((unsigned)mapPixelPos.x); uart_puts(",");
	uart_putdec((unsigned)mapPixelPos.y);
	uart_puts(" Sel=");
	uart_putdec((unsigned)Selected.size());
	uart_puts("\n");
	// No unit selected
	if (Selected.empty()) {
		uart_puts("[RMB] empty selection — bailing\n");
		return;
	}
...
{
	{
		...
		uart_putdec(button);
		uart_puts(" MB=0x");
		uart_puthex((uint32_t)MouseButtons);
		uart_puts(" CursorOn=");
		uart_putdec((unsigned)(int)CursorOn);
		uart_puts(" MouseVP=");
		uart_putdec((unsigned)(uintptr_t)(UI.MouseViewport != nullptr));
		uart_puts(" CState=");
		uart_putdec((unsigned)(int)CursorState);
		uart_puts(" Sel=");
		uart_putdec((unsigned)Selected.size());
		uart_puts(" cur=");
		uart_putdec((unsigned)CursorScreenPos.x); uart_puts(",");
		uart_putdec((unsigned)CursorScreenPos.y);
		uart_puts("\n");
	}
...
{
	{
		...
		uart_putdec((unsigned)Selected.size());
		uart_puts(" BUC=");
		uart_putdec((unsigned)ButtonUnderCursor);
		uart_puts(" OldBUC=");
		uart_putdec((unsigned)OldButtonUnderCursor);
		uart_puts("\n");
	}
	//
	//  Move map.
