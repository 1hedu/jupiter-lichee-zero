...
/* Direct entry point for Jupiter's CallbackMusicTrigger override. The
 * static version stays for the SDL-event-driven path. */
extern "C" {
void Jupiter_CheckMusicFinishedNow()
{
	CheckMusicFinished();
}
}
...
void CallbackMusicTrigger() {
	/* Jupiter port: upstream's MusicFinishedCallback enqueues an SDL_Event
	 * that gets dispatched by the main event loop. Our SDL_PeepEvents stub
	 * silently drops the event, so the chain that runs CheckMusicFinished
	 * → user-level music callback never fires — and the title menu never
	 * gets its TitleScreenMusic kick-off. Fire CheckMusicFinished directly
	 * here. Safe because CheckMusicFinished's only state guard is the
	 * MusicFinishedEventQueued flag, which we don't even reach since we
	 * call CheckMusicFinished from this synchronous trigger. */
