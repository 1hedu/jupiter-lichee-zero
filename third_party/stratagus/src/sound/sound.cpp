...
/* Function-local statics so the maps are constructed on first use,
 * not via __init_array. File-scope ctors of std::map cross-reference
 * the global libstdc++ rb-tree sentinel; on bare-metal with a partial
 * libstdc++ wiring, TU init-order can leave a half-built sentinel that
 * makes the first operator[] insertion loop. Function-local static is
 * the same fix upstream itself uses elsewhere
 * (e.g. third_party/stratagus/src/script_missile.cpp:63,
 *       third_party/stratagus/src/animation.cpp:90,
 *       third_party/stratagus/src/upgrade.cpp:77). */
static std::map<int, LuaActionListener *> &channelMap() {
	static std::map<int, LuaActionListener *> m;
	return m;
}
static std::map<int, sdl2::ChunkPtr> &sampleMap() {
	static std::map<int, sdl2::ChunkPtr> m;
	return m;
}
#define ChannelMap channelMap()
#define SampleMap  sampleMap()
