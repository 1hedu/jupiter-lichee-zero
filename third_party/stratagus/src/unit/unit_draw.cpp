...
void DrawUnitSelection(const CViewport &vp, const CUnit &unit)
{
	...
	/* Jupiter port: with DYNAMIC_LOAD defined, LoadUnitTypeSprite is
	 * skipped and `type.Sprite` is always null — the formula falls
	 * through to subtracting `type.Width/2` extra, shifting the box
	 * up-left by half a frame off the actual unit position. Treat a
	 * missing Sprite as if its Width/Height equaled type.Width/Height,
	 * which collapses the offset to zero. The rendering side already
	 * draws the unit at screenPos via the pc8 bridge using type.Width
	 * / Height as the frame size, so this matches. */
	const int spriteW = type.Sprite ? type.Sprite->Width  : type.Width;
	const int spriteH = type.Sprite ? type.Sprite->Height : type.Height;
	const int x = screenPos.x - type.BoxWidth  / 2 - (type.Width  - spriteW) / 2;
	const int y = screenPos.y - type.BoxHeight / 2 - (type.Height - spriteH) / 2;
...
{
	{
		/* Throttle to once per second — fires every frame per selected
		 * unit otherwise, which floods. Once per ~30 cycles is plenty
		 * to confirm "this is being called" without drowning out other logs. */
		static int sel_count = 0;
		if ((++sel_count & 0x1F) == 1) {
			uart_puts("[SEL] rect color=0x");
			uart_puthex((uint32_t)color);
			uart_puts(" rect=");
			uart_putdec((unsigned)x1); uart_puts(",");
			uart_putdec((unsigned)y1); uart_puts("-");
			uart_putdec((unsigned)x2); uart_puts(",");
			uart_putdec((unsigned)y2);
			uart_puts(" total=");
			uart_putdec((unsigned)sel_count);
			uart_puts("\n");
		}
	}
