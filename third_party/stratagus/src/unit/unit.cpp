...
/* Bare-metal diag helpers for LetUnitDie probe. */
extern "C" {
...
void CUnit::Init(const CUnitType &type)
{
	...
	if (UnitTypeVar.GetNumberVariable()) {
		...
		/* WC1/War1gus port: helpers.lua defines unit Poison spec as
		 * `{ Value = 0, Max = 1000, Enable = true }`, but our Lua type-
		 * lookup ends up with Value=1000 (matching Max) — looks like the
		 * isTable/isNumber dispatch in script_unittype.cpp:1145 takes
		 * the number path. Result: every unit spawns "poisoned" and
		 * HandleBurnAndPoison drains 2 HP/sec until they die. spell-poison
		 * is the only legitimate poison source, and it sets Value at
		 * cast time, so resetting per-unit Value=0 here is safe. */
		static int init_poison_probe = 0;
		if (init_poison_probe < 5) {
			init_poison_probe++;
			uart_puts("[INIT-POI] ");
			uart_puts(type.Ident.c_str());
			uart_puts(" varSize=");
			uart_putdec((unsigned)Variable.size());
			uart_puts(" POISON_INDEX=");
			uart_putdec((unsigned)POISON_INDEX);
			uart_puts(" beforeZero=");
			uart_putdec((unsigned)((unsigned)POISON_INDEX < Variable.size() ? Variable[POISON_INDEX].Value : 999999));
			uart_puts("\n");
		}
		if ((unsigned)POISON_INDEX < Variable.size()) {
			Variable[POISON_INDEX].Value = 0;
		}
	} else {
		Variable.clear();
	}
...
void CUnit::AssignToPlayer(CPlayer &player)
{
	...
	static int assign_poison_probe = 0;
	if (assign_poison_probe < 5) {
		assign_poison_probe++;
		uart_puts("[ASN-POI] ");
		uart_puts(type.Ident.c_str());
		uart_puts(" SaveGameLoading=");
		uart_putdec((unsigned)SaveGameLoading);
		uart_puts(" numVar=");
		uart_putdec((unsigned)UnitTypeVar.GetNumberVariable());
		uart_puts(" stats.Vars[POI].Value=");
		uart_putdec((unsigned)Stats->Variables[POISON_INDEX].Value);
		uart_puts("\n");
	}
	if (!SaveGameLoading) {
		if (UnitTypeVar.GetNumberVariable()) {
			Variable = Stats->Variables;
			/* Same poison-spawn bug as in CUnit::Init: Stats->Variables[]
			 * inherits the helpers.lua mis-parse where Poison.Value=Max=1000.
			 * Zero it here too — AssignToPlayer runs AFTER Init and would
			 * otherwise restore the bad value from the per-player stats copy. */
			if ((unsigned)POISON_INDEX < Variable.size()) {
				Variable[POISON_INDEX].Value = 0;
			}
		}
...
CUnit *UnitOnScreen(int x, int y)
{
	...
	for (CUnit *unitPtr : UnitManager->GetUnits()) {
		...
		const CUnitType &type = *unit.Type;
		/* Jupiter port: DYNAMIC_LOAD leaves type.Sprite null, but the unit
		 * IS visually rendered via the pc8 bridge using type.Width/Height
		 * as the frame size. Treat missing Sprite as if its dims matched
		 * type.Width/Height so the hit box collapses to BoxWidth/Height
		 * centered on the tile center (matches DrawUnitSelection). Without
		 * this every UnitOnScreen call returns null → no UnitUnderCursor →
		 * no magnifier hover indicator + no single-click selection. */
		const int spriteW = type.Sprite ? type.Sprite->Width  : type.Width;
		const int spriteH = type.Sprite ? type.Sprite->Height : type.Height;
		...
		unitSpritePos.x = unitSpritePos.x - type.BoxWidth / 2 -
						  (type.Width - spriteW) / 2 + type.BoxOffset.x;
		unitSpritePos.y = unitSpritePos.y - type.BoxHeight / 2 -
						  (type.Height - spriteH) / 2 + type.BoxOffset.y;
...
{
	{
		static int death_probe = 0;
		if (death_probe < 30) {
			death_probe++;
			uart_puts("[DIE] ");
			uart_puts(unit.Type ? unit.Type->Ident.c_str() : "?");
			uart_puts(" p=");
			uart_putdec((unsigned)unit.Player->Index);
			uart_puts(" tile=");
			uart_putdec((unsigned)unit.tilePos.x); uart_puts(",");
			uart_putdec((unsigned)unit.tilePos.y);
			uart_puts(" hp=");
			uart_putdec((unsigned)unit.Variable[HP_INDEX].Value);
			uart_puts("/");
			uart_putdec((unsigned)unit.Variable[HP_INDEX].Max);
			uart_puts(" cycle=");
			uart_putdec((unsigned)GameCycle);
			uart_puts(" suicide=");
			uart_putdec((unsigned)suicide);
			uart_puts("\n");
		}
	}
