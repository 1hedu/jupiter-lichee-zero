...
/* Bare-metal diag helpers for the CreateUnit probes below. */
extern "C" {
...
static int CclCreateUnit(lua_State *l)
{
	...
	if (unittype == nullptr) {
		uart_puts("[UNIT] CreateUnit bad unittype: ");
		uart_puts(typeName ? typeName : "(nil)");
		uart_puts("\n");
	...
	lua_pop(l, 1);
	if (player == nullptr) {
		uart_puts("[UNIT] CreateUnit bad player for type: ");
		uart_puts(typeName ? typeName : "(nil)");
		uart_puts("\n");
	...
	if (player->Type == PlayerTypes::PlayerNobody) {
		uart_puts("[UNIT] CreateUnit player-nobody for type: ");
		uart_puts(typeName ? typeName : "(nil)");
		uart_puts("\n");
	...
	if (unit == nullptr) {
		uart_puts("[UNIT] CreateUnit MakeUnit nullptr for type: ");
		uart_puts(typeName ? typeName : "(nil)");
		uart_puts("\n");
		LuaDebugPrint(l, "Unable to allocate unit");
		return 0;
	} else {
		bool canBe = UnitCanBeAt(*unit, ipos);
		bool canBuild = unit->Type->Building && CanBuildUnitType(nullptr, *unit->Type, ipos, 0);
		if (canBe || canBuild) {
			unit->Place(ipos);
		} else {
			const int heading = SyncRand() % 256;
		...
		if (unit_count < 30) {
			unit_count++;
			uart_puts("[UNIT] +");
			uart_puts(typeName ? typeName : "?");
			uart_puts(" p=");
			uart_putdec((unsigned)player->Index);
			uart_puts(" tile=");
			uart_putdec((unsigned)ipos.x); uart_puts(",");
			uart_putdec((unsigned)ipos.y);
			uart_puts(" canBe=");
			uart_putdec(canBe);
			uart_puts(" place=");
