...
/* Function-local-static for safe init order on bare-metal (matches the
 * pattern in script_missile.cpp / animation.cpp). File-scope std::map
 * ctors race against other TU statics that lookup into them. */
static std::map<std::string, std::unique_ptr<CUnitType>, std::less<>> &unitTypeMap() {
	static std::map<std::string, std::unique_ptr<CUnitType>, std::less<>> m;
	return m;
}
#define UnitTypeMap unitTypeMap()
