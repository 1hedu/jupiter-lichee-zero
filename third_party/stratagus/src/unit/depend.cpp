...
/// All dependencies hash. Function-local-static for safe TU init order
/// on bare-metal — same pattern used elsewhere in stratagus (e.g.
/// script_missile.cpp:63, upgrade.cpp:77).
static std::map<std::string, DependOrRule, std::less<>> &depends() {
	static std::map<std::string, DependOrRule, std::less<>> m;
	return m;
}
...
std::string PrintDependencies(const CPlayer &player, const ButtonAction &button)
{
	...
	if (!starts_with(button.ValueStr, "unit-") && !starts_with(button.ValueStr, "upgrade-")) {
		/* Action buttons (move/stop/attack/cancel/...) carry empty or
		 * non-unit/upgrade ValueStr. Upstream blocking-printed an error
		 * here every time those buttons were inspected — on Jupiter
		 * (UART blocking, ~2.6 ms per print) that's enough per AI tick
		 * to grind the game to single-digit fps. Silently return no
		 * rules; not a real error. */
		return "";
	}
...
bool CheckDependByIdent(const CPlayer &player, std::string_view target)
{
	...
	if (starts_with(target, "unit-")) {
		...
		/* Empty / non-unit target — same loud-print problem as
		 * PrintDependencies above. AI iteration over force/research
		 * tables and the per-frame button-panel allow check both hit
		 * this with empty/short strings; silently return false so the
		 * UART backpressure doesn't tank the frame rate. */
		return false;
	}
