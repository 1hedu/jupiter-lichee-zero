                ...
                int num)
{
	...
	if (!LogFile) {
		...
		if (LogFile->open(path.string().c_str(), CL_OPEN_WRITE) == -1) {
			// don't retry for each command — was `= false` upstream which
			// re-enables logging after a failed open, causing repeated
			// fs::path / std::to_string allocations every unit command.
