...
{
	/* Jupiter port: "~save/X" comes in from the load menu (load.lua
	 * sets LoadGameFile = "~save/" .. filename). Map that to the FAT
	 * partition's save dir; newlib's fopen routes to FatFs via our
	 * _open syscall stub. */
	const std::string save_prefix = "~save/";
	if (path.compare(0, save_prefix.size(), save_prefix) == 0) {
		return fs::path(std::string("0:/save/") + path.substr(save_prefix.size()));
	}
	if (path[0] == '~') {
...
{
	/* Jupiter port: saves live on the FAT partition under 0:/save/. */
	return fs::path("0:/save/");
}
...
{
	CFile file;
	/* Jupiter port: saves go to the FAT partition under 0:/save/. The
	 * CL_WRITE_GZ flag tacks ".gz" on, gzip's deflate buffers properly
	 * onto the FatFs sector cache. */
	std::string fullpath = std::string("0:/save/") + filename;
...
{
	/* Jupiter port: drop the file from the FAT partition. SaveGame()
	 * appends ".gz" via CL_WRITE_GZ; unlink both shapes to be safe. */
	std::string with_gz = std::string("0:/save/") + filename + ".gz";
	std::string no_gz   = std::string("0:/save/") + filename;
	unlink(with_gz.c_str());
	unlink(no_gz.c_str());
}
