...
*/
/* Wrapped in a function so the std::map initializes on first use rather
 * than at static-init time. Our bare-metal Jupiter port doesn't run
 * .init_array (libstdc++ EH machinery deadlocks a bunch of TU-level
 * ctors) so TU-level std::map<> globals end up empty. Function-local
 * static with -fno-threadsafe-statics bypasses that — the map constructs
 * the first time something calls MissileClassNames(). */
static const std::map<std::string_view, MissileClass>& MissileClassNames() {
	static const std::map<std::string_view, MissileClass> m = {
...
void MissileType::Load(lua_State *l)
{
	...
	for (lua_pushnil(l); lua_next(l, 2); lua_pop(l, 1)) {
		...
		if (value == "File") {
			...
			const auto& names = MissileClassNames();
			const auto it = names.find(className);
			if (it != names.end()) {
