...
#include "unittype.h"
#include "upgrade.h"
/* bare-metal port: network/ not compiled in */
/* #include "network.h" */
...
{
	uart_puts("[ARM] CheckingWork\n");
	AiCheckingWork();
	uart_puts("[ARM] Supply\n");
	if (!AiPlayer->NeedSupply && AiPlayer->Player->Supply == AiPlayer->Player->Demand) {
		AiRequestSupply();
	}
	uart_puts("[ARM] CollectGate ");
	uart_putdec((unsigned)((GameCycle / CYCLES_PER_SECOND) % COLLECT_RESOURCES_INTERVAL));
	uart_puts(" vs ");
	uart_putdec((unsigned)((unsigned long)AiPlayer->Player->Index % COLLECT_RESOURCES_INTERVAL));
	uart_puts("\n");
	if ((GameCycle / CYCLES_PER_SECOND) % COLLECT_RESOURCES_INTERVAL ==
		(unsigned long)AiPlayer->Player->Index % COLLECT_RESOURCES_INTERVAL) {
		uart_puts("[ARM] CollectResources enter\n");
		AiCollectResources();
		uart_puts("[ARM] CollectResources exit\n");
	}
	uart_puts("[ARM] CheckRepair\n");
	AiCheckRepair();
