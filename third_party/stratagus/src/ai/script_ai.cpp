...
/* bare-metal port: network/ not compiled in (AI code doesn't call it) */
#include "stratagus.h"
...
/* bare-metal port: AiProcessor* are an RL research feature that talks to an
 * external Python AI agent over TCP. Network/ is not compiled in and WC1
 * scripts never call these. Stubbed to return nil/0. */
static int CclAiProcessorSetup(lua_State *l) { (void)l; lua_pushnil(l); return 1; }
static int CclAiProcessorStep(lua_State *l)  { (void)l; lua_pushnil(l); return 1; }
static int CclAiProcessorEnd(lua_State *l)   { (void)l; return 0; }
