...
-- Lua 5.1/5.2 compat: helpers.lua, icons.lua, the menus and War1gus's
-- DefineUnitFromSpec all still call the table.* iterators that were removed
-- in Lua 5.3. Polyfill them once here so every downstream Load() sees them.
...
-- The map/scenario editor (editor/*.cpp) is out of scope for the bare-metal
-- port. Desktop Stratagus exposes the `Editor` global + `EditorRunningType`
-- enum via tolua (editor.pkg); we skip those bindings since none of the
-- editor C++ is linked. guichan.lua's top-level branch reads
-- `Editor.Running == EditorCommandLine` to decide which menu to launch, so
-- declare the shape with Running=NotRunning to force the canonical title-
-- menu path. SetGameMode / RunEditorMenu remain undefined; nothing in the
-- gameplay path reaches them.
EditorNotRunning  = 0
EditorStarted     = 1
EditorCommandLine = 2
EditorEditing     = 4
...
-- Jupiter port: unlock all 12 campaign missions out of the gate. The
-- canonical CampaignButtonState gates visible buttons on
-- preferences.Campaign{Orc,Human}, which would normally start at 1 and
-- increment on victory. Without a writable preferences.lua we can't
-- persist progress, so testing each mission requires every button up
-- front. Bump the defaults to 12 (the full mission count from
-- CreateCampaign in scripts/campaigns.lua).
DefaultPreference("CampaignOrc", 12)
DefaultPreference("CampaignHuman", 12)
...
if preferences.PlayIntro == true then
   -- Jupiter port: skip the file_exists() guards. Upstream's
   -- ListFilesInDirectory("videos") resolves under StratagusLibPath,
   -- but our cinematics live at /videos/<name>.vbin on the FatFs
   -- partition (same place as /intros/), which that walker can't see.
   -- Always register the four canonical cinematics and let
   -- war1_movie_play decide per-file: if the .vbin is present it plays,
   -- otherwise PlayMovie returns -1 and ShowTitleImage's static-image
   -- fallback no-ops on a video path.
   SetTitleScreens(
      {Image = "ui/logo.png",
