...
-- Jupiter port: Video.Width is 0 at ui.lua load time (CclSetVideoResolution
-- skips the write outside CclInConfigFile), so the upstream "Video.Width
-- - N" formulas resolved to negative or zero, parking the resource icons
-- against the minimap. Hardcode the LCD width here. SCREEN_W mirrors the
-- 480x272 panel the bridge forces; if the panel ever changes, change this
-- in lockstep.
local SCREEN_W = 480
...
UI.Resources[1].IconX = SCREEN_W - 137 - 9
UI.Resources[1].IconY = 1
UI.Resources[1].TextX = SCREEN_W - 129 - 9 - 40
...
UI.Resources[2].IconX = SCREEN_W - 55 - 13 - 13
UI.Resources[2].IconY = 0
UI.Resources[2].TextX = SCREEN_W - 55 - 13 - 40
...
UI.Resources[FoodCost].IconX = SCREEN_W - 0 - 13 - 13
UI.Resources[FoodCost].IconY = 0
UI.Resources[FoodCost].TextX = SCREEN_W - 0 - 13 - 40
...
UI.Resources[FreeWorkersCount].TextX = -(SCREEN_W - 14 - 8 -20)
UI.Resources[FreeWorkersCount].TextY = 28
UI.Resources[FreeWorkersCount].IconX = SCREEN_W - 14 - 8 - 21
...
function LoadUI(race, screen_width, screen_height)
   ...
   Load("scripts/widgets.lua")
   UI.Fillers:clear()
   -- Jupiter port: same AddFiller structure as upstream — Resize is a
   -- no-op in our pc8 path so the asset's native size is what renders
   -- (we picked prescaled assets sized for 480x272). Hardcode SCREEN_W/H
   -- below because Video.Width/Height are 0 when LoadUI runs (same
   -- script-load timing as the resource-icon fix above), which would
   -- park the right/bottom panels off-screen.
   local SCREEN_W = 480
   local SCREEN_H = 272
   AddFiller("ui/" .. race .. "/minimap_2.png",                0,                 0,  72,  72)
   AddFiller("ui/" .. race .. "/left_panel_200.png",           0,                72,  72, 200)
   AddFiller("ui/" .. race .. "/bottom_panel_400_flipped.png", 72,                0, 400,  12)
   AddFiller("ui/" .. race .. "/right_panel_272.png",          SCREEN_W - 8,      0,   8, 272)
   AddFiller("ui/" .. race .. "/bottom_panel_400.png",         72, SCREEN_H - 12, 400,  12)
