...
function RunSinglePlayerSubMenu()
  ...
  -- Jupiter port: Solo Game is in scope but its menu uses DropDownWidget
  -- (race / resources / opponents / units / gametype) which only has the
  -- container bound, not the LuaListModel adapter — so the menu hangs in
  -- gcn::DropDown::draw on a not-fully-wired list. No-op the button until
  -- the DropDown stack lands. Same pattern as Multi Player / Editor.
  menu:addFullButton("~!Solo Game", "s", offx + 96, offy + 52 + 18*2, function() end)
  menu:addFullButton("~!Campaign Game", "c", offx + 96, offy + 52 + 18*3,
    function() RunCampaignGameMenu(); menu:stop(1) end)
  -- Load / Replay also need save/replay file IO which we don't have on
  -- bare-metal yet — no-op for now.
  menu:addFullButton("~!Load Game", "l", offx + 96, offy + 52 + 18*4, function() end)
  menu:addFullButton("~!Replay Game", "r", offx + 96, offy + 52 + 18*5, function() end)
...
function BuildProgramStartMenu()
  ...
  -- Multi-player is out of scope for the bare-metal port (no IP stack):
  -- the button stays in the canonical layout for visual parity with
  -- upstream WC1 but the click is a no-op. Keeping menu:stop off so the
  -- title menu doesn't tear down on a click that has nowhere to go.
  menu:addFullButton("~!Multi Player Game", "m", offx + 96, offy + 52 + 17*3, function() end)
  menu:addFullButton("~!Options", "o", offx + 96, offy + 52 + 17*4, function() RunOptionsSubMenu(); menu:stop(1) end)
  -- Editor likewise — editor/*.cpp isn't ported (see project scope), and
  -- Editor.Running is a port-side shim. Click is a no-op.
  menu:addFullButton("~!Editor", "e", offx + 96, offy + 52 + 17*5, function() end)
...
-- Network multiplayer is out of scope for the bare-metal port (no IP stack).
-- Skipping network.lua also avoids its Lua 5.3 string-literal parse error
-- (line 246 uses `\.` inside a string, which 5.1 silently accepted but 5.3
-- treats as an invalid escape).
