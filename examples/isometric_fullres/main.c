...
/* Full-resolution: 480×272, no doubling. Every pixel sampled individually.
 * With D-cache working (SMP=1), this runs at ~2.5ms — still 60fps. */
...
static void render_ground(uint32_t fb_addr)
{
    ...
    int32_t du_x =  ca / ISO_SCALE;
    int32_t dv_x = -sa / ISO_SCALE;
    int32_t du_y = sa * 2 / ISO_SCALE;
    int32_t dv_y = ca * 2 / ISO_SCALE;
    ...
    for (uint32_t sy = 0; sy < LCD_H; sy++) {
        int32_t u = u_row;
        int32_t v = v_row;
        uint32_t *row = fb + sy * LCD_W;

        for (uint32_t sx = 0; sx < LCD_W; sx++) {
            uint32_t tx = (uint32_t)(u >> FP) & MAP_MASK;
            uint32_t ty = (uint32_t)(v >> FP) & MAP_MASK;
            uint8_t tile = world_map[ty][tx];
            row[sx] = tile_colors[tile * 2 + ((tx ^ ty) & 1)];
            u += du_x;
            v += dv_x;
        }
...
void main(void)
{
    uart_puts("\n\n=== Jupiter SDK — 2.5D Isometric Diorama (FULL-RES) ===\n");
