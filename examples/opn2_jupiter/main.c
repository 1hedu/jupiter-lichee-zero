...
static void hud_waveform(volatile uint32_t *ovl)
{
    ...
    for (int i = 0; i < num_pts; i++) {
        ...
        /* Map to pixel offset from center (4× gain for typical Genesis levels) */
        int y_off = (mono * inner_h) / 8192;
        if (y_off > inner_h) y_off = inner_h;
        if (y_off < -inner_h) y_off = -inner_h;
        int px = WAVE_X + 2 + i;
        int py = mid_y - y_off;
...
void main(void)
{
    ...
    while (1) {
        ...
        /* 1) Render space + planet + stars to VI0 back buffer */
        render_space_full((uint32_t *)back_fb);
        render_planet((uint32_t *)back_fb, frame);
        render_stars((uint32_t *)back_fb, frame);
