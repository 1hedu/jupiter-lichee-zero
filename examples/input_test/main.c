/*
 * NES Controller Input Test — Four systems, one D-pad
 *
 * Same tri-layer layout, all sprites controlled by NES controller.
 * Wiring: PG0=OUT, PG1=CLK, PG2=D0, GND, 3.3V
 *
 * Build: make GAME=examples/input_test/main.c
 */
#include "jupiter.h"
#include "nes.h"
#include "gb.h"
...
int main(void)
{
    ...
    /* ---- Genesis: runtime ARGB→4bpp conversion ---- */
    /* Genesis ARGB scratch in VRAM at 0x43D00000 (1 MB free between
     * OVL1 at 0x43C00000+1MB and cedar's region at 0x43E00000).
     * Was 0x43400000 — that's INSIDE the CODE bss region, fine when
     * BSS was small but collides with menu-built BSS once it grows
     * past 0x43400000 (causes flaky launches: ARGB copy stomps on
     * other demos' static state). VRAM is safe — no static data
     * lives there. */
    ...
    while (1) {
        ...
        /* Debug: print Genesis raw data every 60 frames */
        {
            static int dbg_cnt = 0;
            if (++dbg_cnt >= 60) {
                dbg_cnt = 0;
                uart_puts("[gen] btn=0x"); uart_puthex(gen_pad.buttons);
                uart_puts(" PB=0x"); uart_puthex(REG32(PIO_BASE + 0x34) & 0xFF);
                uart_puts(" PG=0x"); uart_puthex(REG32(PIO_BASE + 0xE8) & 0xFF);
                uart_puts("\n");
            }
        }
        ...
        /* N64 analog stick (deadzone ~20, scale to ±3) */
        if (n64_pad.stick_x > 20)       dx = n64_pad.stick_x / 25;
        else if (n64_pad.stick_x < -20) dx = n64_pad.stick_x / 25;
        if (n64_pad.stick_y > 20)       dy = -n64_pad.stick_y / 25;  /* Y inverted */
        ...
        /* Apply movement with clamping per viewport */
        gb_x += dx;
        gb_y_off += dy;
        if (gb_x < 0) gb_x = 0;
        ...
        /* Advance animation frames only while moving */
